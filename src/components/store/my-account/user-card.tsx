...
const css = {
  wrapper: `w-full h-16 min-[430px]:h-auto flex gap-4 relative round-lg bg-white`,
  wrapperImg: `h-17 min-w-17 w-17 relative bg-gray-100 round-full border-none`,
      ...
      {!seeEditMode ? (
        <div className="h-max flex flex-col gap-4">
          <div className={`${css.wrapper} justify-between`}>
            ...
            {/* para mobile */}
            <div className={`min-[430px]:hidden h-16 flex items-center justify-center`}>
              <h3 className={`${css.name}`}>{userData.name}</h3>
            </div>
            <EditButton setSeeEditMode={setSeeEditMode} />
          </div>
          {/* para mobile */}
          <p className={`${css.email} min-[430px]:hidden`}>Email: {userData.email}</p>
        </div>
      ) : (
