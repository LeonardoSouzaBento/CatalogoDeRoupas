      ...
      <div className={`${css.card}`}>
        <div className={`${css.container}`}>
          {plans.map((plan, index) => (
            <div
              key={index}
              className={cn(css.wrapper, plan.emphasis && css.emphasis)}
            >
              <h4 className={plan.emphasis ? "text-white" : ""}>
                {plan.title}
              </h4>
              <p
                className={
                  plan.emphasis ? "text-white" : "text-muted-foreground"
                }
              >
                {plan.subtitle}
              </p>
              <ul className={`mb-2 last:mb-0`}>
                {plan.features.map((item, index) => (
                  <li
                    key={index}
                    className={cn(css.item, plan.emphasis && "text-white")}
                  >
                    <CheckIcon section={plan.emphasis ? "plans" : undefined} />{" "}
                    {item}
                  </li>
                ))}
              </ul>
